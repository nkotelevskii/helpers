...
class PreResNetSpectral(nn.Module):
    def __init__(self, num_classes=10, depth=110):
        super(PreResNetSpectral, self).__init__()
        if depth >= 44:
            assert (depth - 2) % 9 == 0, "depth should be 9n+2"
            n = (depth - 2) // 9
            block = Bottleneck
        ...
        def add_sn(m):    
            if isinstance(m, (nn.Conv2d, nn.ConvTranspose2d, nn.Linear)):
                return spectral_norm(m, n_power_iterations=5)
            else:
                return m
...
class PreResNet20Spectral:
    base = PreResNetSpectral
    args = list()
    kwargs = {"depth": 20}